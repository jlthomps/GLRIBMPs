...
RMprep <- function(df,
                   ...
                   cnames.in="",
                   cnames.new="rain",
                   tz="")                #time zone. CST6CDT for central time.
                                         #for other times use values in the TZ* column here:
...
#                  time="pdate"     # column with as.POSIXctdate
#                  ) 
...
#                  depth="depth",  rain depth in event file
...
#                sdate,          Start date column in df.events rain file as POSIX
#                edate,          End date column in df.events rain file as POSIX
#                depth,          column in df.events with event rain depth
...
RMevents.plot <- function(df,date="pdate",rain = "rain",
  ...
  for (i in 1:(nrow(df.events))) {
    p.sdate <- as.as.POSIXct(df.events[i,sdate] - plot.buffer*24*3600/2)
    p.edate <- as.as.POSIXct(df.events[i,edate] + plot.buffer*24*3600/2)
    ...
    arrows(as.POSIXct(df.events[i,sdate]),(rmax-0.15),
...
#                date,           Date column in df as POSIXct
#                Qdate,          Date column in dfQ as POSIXct
#                rain,           Column in df with instantaneous rain values
#                Q,              Column in dfQ with instantaneous Q values
#                df.events,      Data frame with start and end dates/times for events
#                sdate,          Start date column in df.events rain file as POSIXct
#                edate,          End date column in df.events as POSIXct
#                erain,          Event rainfall depth column in df.events
#                plot.buffer,    Used to define plotting window in days. Graphs will include
#                                data Time period preceding beginning of event for including
#                                in the graphs
...
RMevents.plotQ <- function(df,dfQ,date="pdate",Qdate="pdate",rain = "rain",Q="Q",
  ...
  for (i in 1:(nrow(df.events))) {
    ########################## Graph Precip  ###########################################
    p.sdate <- as.as.POSIXct(df.events[i,sdate] - plot.buffer*24*3600/2,tz="")
    p.edate <- as.as.POSIXct(df.events[i,edate] + plot.buffer*24*3600/2,tz="")
    subdf <- subset(df, df[,date]>=p.sdate & df[,date]<=p.edate)
    rmax <- max(subdf[,rain] + 0.3)
    subrain <- subdf[,rain]
    subdate <- as.POSIXct(subdf[,date])
