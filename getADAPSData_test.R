...
getADAPSData_test <- function(siteNo,StartDt,EndDt,dataFile="") {
if (nchar(dataFile)<=3) {
...
if ((length(unique(POR$parameter_cd))+length(unique(POR$parameter_cd)))>=3) {
  if (max(POR$startDate)<=StartDt&min(POR$endDate)>=EndDt) {
    ...
    colnames(adaps_data_all) <- c("datetime","agency_cd","site_no","X01_00065","X02_00060","X05_99909")
  ...
  colnames(adaps_data_all) <- c("datetime","X05_99909","X01_00065","X02_00060","agency_cd","site_no")
}
