...
#' plotSteps(steps,DT)
plotStepsGLRIAll <- function(steps,localDT,transformResponse="lognormal"){
  ...
  for (i in 2:nSteps){
    ...
    frozen <- as.numeric(localDT[,"frozen"])
    
    df <- data.frame(obs=responseValue, pred=modelReturn$YPRED,frozen=frozen,stringsAsFactors=FALSE)
    ...
    plot(df$pred,df$obs,ylab="Observed",xlab="Predicted", pch=df$frozen,
