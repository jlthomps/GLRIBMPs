...
for (i in 1:length(sites)) {
  ...
  if (nrow(flowData)>0) {
    ...
    minDate <- as.character(format(minDate,"%Y-%m"))
    maxDate <- as.character(format(maxDate,"%Y-%m"))
    cat(Q90,i,site,sep=" ")
  } else {
  ...
  if (nrow(flowData2)>0 & max(flowData2$X_00060_00003,na.rm=TRUE)!=-Inf) {
    colnames(flowData2) <- c("agency_cd","site_no","dateTime","tz_cd","remark","flow")
  ...
  barbStats <- as.data.frame(cbind(site,Q90,Q10,Q50,Qmax,Qmin,minDate,maxDate,QinstMax,l7Q2,l7Q10,Q72lm,Q710lm),stringsAsFactors=FALSE)
  BarbStats[[i]] <- barbStats
}

allBarbStats <- do.call(rbind,BarbStats)
write.table(allBarbStats,file="allBarbStatsMultiYr.txt",sep="\t",row.names=FALSE)
